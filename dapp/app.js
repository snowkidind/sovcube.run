...
async function timelockTokens(contractAddress, amount) {
    try {
        const bsovTokenContract = new web3.eth.Contract(bsovTokenABI, bsovTokenAddress);
        await bsovTokenContract.methods.approveAndCall(contractAddress, amount, "0x").send({ from: selectedAccount });
    } catch (error) {
        if (error.message.includes("HighFees")) {
            document.getElementById('errorMessage').innerText = 'Absurdly high ETH fees detected. Something is wrong with the parameters you have specified, or the network fees are currently too high.';
        } else {
...
// document.getElementById('timelock1').style.display = 'none';
// document.getElementById('timelock2').style.display = 'none';}

...
contract TimelockAndRewardsContractT2 is ReentrancyGuard {
    ...
    // Events
    event SentLockedTokensToSingle(
        address indexed from,
        address indexed to,
        uint256 amount
    );
    event SentLockedTokensToMany(
        address indexed from,
        address[] receivers,
        uint[] amounts
    );
    ...
    constructor(address _tokenContractAddress) {
        ...
        owner = msg.sender;
        lockExpirationDateRegularAccount = (block.timestamp +
            lockExpirationForAllRegularAccounts);
        currentGlobalTier = 1;
    }
    ...
    function ownerSeedContract() public onlyOwner {
        // this file is modified to lower the max amount of seeded BSOV to 10000 expose end of life issues
        uint256 balance = tokenContract.balanceOf(address(this));
        require(
            tokenContract.approveAndCall(address(this), balance, "0x"),
    ...
    function getRewardRatioForTier(
        uint256 tier
    ...
    function receiveApproval(
        ...
        require(
            _tokenContract == address(tokenContract),
            "Can only deposit BSOV into this contract!"
        );
        require(
            _value > 100,
            "Value must be greater than 100 Mundos, (0.00000100 BSOV)"
        );
        require(
            ERC20Interface(tokenContract).transferFrom(
                _sender,
                address(this),
                _value
            ),
    ...
    function calculateAndSendRewardsAfterTimelock(
        address user,
        uint256 amountTimelocked
    ) internal {
        require(
            amountTimelocked <= 14500000000000,
        ...
        if (totalCumulative < nextTierThreshold || currentTier == 10) {
            ...
            newlyEarnedRewards +=
                (amountInNextTier * rewardRatioNext) /
                TOKEN_PRECISION;
        }
    ...
    function sendLockedTokensToSingle(
        address _receiver,
        uint _amount
    ) public nonReentrant {
    ...
    function sendLockedTokensToMany(
    ...
    function acceptUntakenIncomingTokens() public nonReentrant {
        require(
            ...
            "You have no Incoming Tokens to accept!"
        );

        uint256 incomingTokensAmount = balanceUntakenIncomingAccount[
            msg.sender
        ];
        balanceIncomingAccount[msg.sender] += incomingTokensAmount;
        lockExpirationForUserIncomingAccount[msg.sender] =
            block.timestamp +
            resetTimeLeftIncomingAccount;
    ...
    function calculateMaxWithdrawable(
        ...
        uint elapsedWithdrawalPeriods = (block.timestamp - lastWithdrawalTime) /
            timeBetweenWithdrawals;
    ...
    function getUnlockedForWithdrawalRegularAccount(
        address user
    ) public view returns (uint) {
        return calculateMaxWithdrawable(lastWithdrawalRegularAccount[user]);
    }

    function getUnlockedForWithdrawalIncomingAccount(
        address user
    ) public view returns (uint) {
        return calculateMaxWithdrawable(lastWithdrawalIncomingAccount[user]);
    }

    function getBalanceRegularAccount(
        address _addr
    ) public view returns (uint256 _balance) {
        return balanceRegularAccount[_addr];
    }

    function getBalanceUntakenIncomingAccount(
        address _user
    ) public view returns (uint256) {
        return balanceUntakenIncomingAccount[_user];
    }

    function getBalanceIncomingAccount(
        address _addr
    ) public view returns (uint256 _balance) {
        return balanceIncomingAccount[_addr];
    }

    function getLastWithdrawalIncomingAccount(
        address _addr
    ) public view returns (uint256 _lastWithdrawalTime) {
        return lastWithdrawalIncomingAccount[_addr];
    }

    function getLastWithdrawalRegularAccount(
        address _addr
    ) public view returns (uint256 _lastWithdrawal) {
        return lastWithdrawalRegularAccount[_addr];
    }

    function getTimeLeftRegularAccount()
        public
        view
        returns (uint256 _timeLeft)
    {
        require(
            lockExpirationDateRegularAccount > block.timestamp,
            "Tokens are unlocked and ready for withdrawal"
        );
        return lockExpirationDateRegularAccount - block.timestamp;
    }

    function getTimeLeftIncomingAccount(
        address _addr
    ) public view returns (uint256 _timeLeft) {
        if (lockExpirationForUserIncomingAccount[_addr] <= block.timestamp) {
            return 0;
        } else {
            return
                lockExpirationForUserIncomingAccount[_addr] - block.timestamp;
        }
    }
}
